 ...
 "cells": [
  ...
  },
  {
   ...
   "source": [
    ...
    "        pass\n",
    "\n",
    "    # Fallback to api.binance.us if api.binance.com is blocked or fails (essential for US-based Google Colab IPs)\n",
    "    url_us = f\"https://api.binance.us/api/v3/klines?symbol={symbol}&interval={interval}&limit={limit}\"\n",
    "    if start_time: url_us += f\"&startTime={start_time}\"\n",
    "    try:\n",
    "        res = requests.get(url_us, timeout=10)\n",
    "        if res.status_code == 200:\n",
    "            df = pd.DataFrame(res.json(), columns=['Timestamp', 'Open', 'High', 'Low', 'Close', 'Volume', 'C1', 'C2', 'C3', 'C4', 'C5', 'C6'])\n",
    "            df[['Open', 'High', 'Low', 'Close', 'Volume']] = df[['Open', 'High', 'Low', 'Close', 'Volume']].apply(pd.to_numeric)\n",
    "            return df[['Timestamp', 'Open', 'High', 'Low', 'Close', 'Volume']]\n",
    ...
    "    \n",
    "    if master_df.empty:\n",
    "        raise ValueError(\"Could not fetch any data from Binance APIs. Please check your internet connection or IP geoblocks.\")\n",
    "    \n",
  ...
  },
  {
   ...
   "source": [
    ...
    "print('\\n--- FORWARD PROJECTION ---')\n",
    "for i in range(5):\n",
    "    p = last_price * np.exp(pred_returns[-1, i])\n",
    "    direction = 'UP' if pred_returns[-1, i] > 0 else 'DOWN'\n",
    "    print(f'T+{i+1}m: {p:.2f} ({direction})')\n",
    "    last_price = p"
   ],
   "id": "8e9a7531a02b82a3",
   "outputs": [],
